...
{
	IRQn_t	 irqn;
	uint32_t irq_priority, clk = sysclks.apb1_timer_freq;
		
	if (tmr == _TIM2) {
		// register callback function
		callback2 = cb; 
		// enable timer clocking
		_RCC->APB1ENR = (_RCC->APB1ENR & ~(1 << 0)) | (1 << 0);
	} else if (tmr == _TIM3) {
		// register callback function
		callback3 = cb;
		// enable timer clocking
		_RCC->APB1ENR = (_RCC->APB1ENR & ~(1 << 1)) | (1 << 1);
	} else if (tmr == _TIM4) {
		// register callback function
		callback4 = cb ;
		// enable timer clocking
	    _RCC->APB1ENR = (_RCC->APB1ENR & ~(1 << 2)) | (1 << 2);
	} else if (tmr == _TIM5) {
		// register callback function
		callback5 = cb ;
		// enable timer clocking
	...
	// set mode
	tmr->CR1 &= ~(1 << 7); // pour désactiver
	tmr->CR1 |= (1 << 7); // pour activer
